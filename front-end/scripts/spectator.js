...
let maxScore = 0;
let showLiveUpdates = true;

const timer = () => {
  const secondsRaw = timeRemaining(localStorage.getItem("endTime"));
  if (secondsRaw <= 15) {
    showLiveUpdates = false;
    document.getElementById('overlay').classList.add('stopLiveUpdates'); 
  }
    ...
    if (showLiveUpdates) {
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        if (users[i]._id == data._id) {
          ...
          users[i].liveUpdateScore = data.score;
          dataIndex = i;
          break;
        }
      }
      ...
      for (let i = dataIndex - 1; i >= 0; i--) {
        ...
        if (users[i].liveUpdateScore < users[i + 1].liveUpdateScore) {
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        progress.style.width = `${(users[i].liveUpdateScore/(((maxScore/1000) + 1) * 1000)) * 100}%`;
      }
    }
  });
}
    ...
    data.json().then((object) => {
      ...
      if (currentState === "ready") {
        ...
        if (!localStorage.getItem("endTime")) {
          getServerOffset((serverOffset) => {
            localStorage.setItem("endTime", object.endTime - serverOffset);
            timer();
          })
        } else {
          timer();
      ...
      Object.keys(object.users).forEach((key) => {
        ...
        if (currentState === "ready") {
          val.score = 0;
          val.liveUpdateScore = 0;
        }
        if (val.gameMode === "playing")
          users.push(val);
      });
      users.sort((a, b) => b.liveUpdateScore - a.liveUpdateScore)
      if (users.length !== 0) {
        maxScore = users[0].liveUpdateScore;
      }
...
const addUser = (user, index) => {
  ...
  progress.style.width = `${(user.liveUpdateScore/(((maxScore/1000) + 1) * 1000)) * 100}%`;

  const score = document.createElement('div');
  score.classList.add('score');
  score.id = `score${user._id}`;
  score.innerHTML = user.liveUpdateScore;
  console.log(`ADDING USER, SCORE: ${user.liveUpdateScore}`);
